...
public class Game extends ConsoleView {
    ...
    @Override
    public void run() {
        c = new Console(400, 300, 12, "Space Invaders!", this);
...
class AlienGrid {
            ...
            for (int col = 0; col < cols; col++) {
                if (!isAlienDead[row][col]) {
                    ...
                    if (Math.sqrt((yloc - by) * (yloc - by) + (xloc - bx) * (xloc - bx)) <= ALIEN_WIDTH / 2) {
                        if (Game.DEBUG) System.out.println("hit " + row + " " + col);
    ...
    private void resetgrid() {
        ...
        if (Game.DEBUG) System.out.println(leftcol + " " + rightcol + " " + toprow + " " + botrow);
    }
